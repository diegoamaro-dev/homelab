...
Overall health:
Degraded — **with 34 open audit findings, one of which is currently live** (H-2 is now
**half** closed — the non-secret half at I-5, the secret half still open as M-D — so the
count is unchanged). Backup
**retention grouping is fixed** (I-4, 2026-07-31; retention deliberately held at
`--dry-run`) and **holding across twenty-one further unattended nights**; **backup coverage was
...
with `degrades=[]` for the first time in that sequence, and `health.json` shows
`audit.status ok`, `age_days 0` — so that second degradation **appears to have cleared**. One
night of evidence, recorded at the I-6 closeout as an observation for a future session to
confirm; the finding is **not** closed on this basis. **The LAN is now
...
H-1c — `bin/backup-probe` still cannot see retention outcomes, script exit status, snapshot
count, lock state or missed nights). **S-8 depends on S-7**, the open zero-cost Health
Aggregator decision, which must be taken first.
**I-8 is COMPLETE 2026-08-20** — the backup mechanism is now version-controlled. Both
`/usr/local/bin/homelab-backup.sh` (sha256 `330df064…5895a554`) and its scheduler
`/etc/cron.d/homelab-backup` (sha256 `976aa694…7ebc8756`) are captured **byte-identically**
under `07_operations/backups/`, mirroring the host paths. They are **Recovery Artifacts, not
a deployment source**; the script is committed **non-executable (0644) as a deliberate
inertness control** while the live file stays `0755`. **I-8 closes version-control durability,
not restic coverage** — `07_operations/` is not in the restic path set, so **H-2 is
unchanged**. Closeout `09_logs/2026-08-20_I8_backup_script_tracked.md`.
**I-6 is COMPLETE 2026-08-20** — the D-1.5 anchor now has real, snapshot-level protection:
it carries `["anchor","d15-rollback"]` and is **outside `--tag nightly` scope**, so no
`forget --tag nightly` can reach it. **The anchor is now `42506e44`**; the retag replaced the
snapshot, and `63c072f4` survives only as its `original` provenance field.
**G-I6-8 PASS** on the first unattended cycle, all fourteen predictions observed
(`09_logs/2026-08-20_I6_closeout.md`). **I-5 is COMPLETE 2026-08-19** — backup coverage
extended from 13 to **16 recorded paths**, closing the non-secret half of H-2; **G-I5-9 PASS**
on the first unattended cycle (`09_logs/2026-08-19_I5_closeout.md`). **I-4 is COMPLETE
2026-07-31** — the restic
grouping defect is fixed and Gate 8 closed on real evidence (45 snapshots / **42 groups**,
byte-identical path sets, parent detection restored, zero removals, no locks). Retention stays
`--dry-run`. Then S-8, and only then
S-10 — the single irreversible step in the whole roadmap. **The S-10 input now exists:** the
first would-remove report landed **2026-08-05** exactly as I-4 predicted and reached
**11 snapshots** by 2026-08-19, holding at 11 through **2026-08-20**, with **nothing removed**
— see *Backups* below. **S-10's I-6 precondition is now satisfied**; S-10 itself remains Open
and unapproved. **Correction, proven at I-5:** editing the `PATHS` array starts a new group but
does **not** restart that report from zero — `restic forget` evaluates each `host,paths` group
independently, so the would-remove set was byte-identical across the boundary. The 11
proposed snapshots now sit in a group **frozen at 22 members that no future snapshot can
join**. **S-1 (LAN trust posture) is
...
Last completed:
**I-8 — track the backup mechanism in the repository (N-5) — COMPLETE 2026-08-20.** The
nightly backup script existed in exactly one place, on the root disk, and had **never** been
in Git — verified at capture: zero matches in the working tree and **zero add-commits across
all history**. It was also outside the restic path set, so **the script that creates the
backups was neither backed up nor version-controlled**, and it held decisions that existed
nowhere else in executable form (the undated `SNAP_DIR`, the 16 static paths, the deliberate
`--dry-run`, the deliberate absence of `--prune`). **Scope widened by operator decision:** the
scheduler `/etc/cron.d/homelab-backup` was equally untracked, and capturing the script without
it would have produced a recovery that restores a script nothing invokes — so **both** were
captured, byte-identically, under `07_operations/backups/`, mirroring the host paths
(`330df064…5895a554` and `976aa694…7ebc8756`). **No redaction was needed** — neither file
contains a secret value. They are **Recovery Artifacts, not a deployment source**; the script
is committed **non-executable (0644) as a deliberate inertness control** while the live file
remains `0755`, because running a *drifted* copy would create a real snapshot with a wrong
path set and open a spurious group, polluting the S-10 input. **G-I8-1…8 all PASS**, including
**G-I8-4/5 — zero production change**, evidenced by a live baseline recorded before capture and
compared after: both host files byte- and mtime-unchanged, no `restic` command run, cron
untouched. **No overnight gate exists for I-8 by design** — it changed nothing in production,
so a nightly cycle had nothing to prove. **I-8 closes version-control durability, not restic
coverage: H-2 is unchanged.** **Standing obligation:** whenever either live file changes, the
tracked copy must be updated in Git in the same change.
Closeout: `09_logs/2026-08-20_I8_backup_script_tracked.md`. Prior:
**I-6 — give the D-1.5 anchor real protection (N-2) — COMPLETE 2026-08-20.** The anchor was
tagged `nightly` like every ordinary snapshot and survived only by accident — `--dry-run` plus
a dated legacy group no future snapshot can join. **Decision B (operator-approved 2026-08-19):
snapshot-level protection.** A single `restic tag --set anchor,d15-rollback` moved it out of
`--tag nightly` scope entirely, so protection lives **in the snapshot**, not in the
invocation — no `forget --tag nightly`, scripted or ad-hoc, can reach it. **The retag replaced
the snapshot: the anchor is now `42506e44`, and `63c072f4` no longer names any snapshot** — it
is preserved as the new snapshot's `original` field and stays permanently resolvable, so no
historical record needed rewriting. `restic` offers **no ID-preserving protection mechanism**;
the identity cost was characterized on a disposable repository and accepted in advance.
**The backup script was deliberately NOT modified**, so **I-8's tracking target is unchanged at
`330df064…5895a554`**. **P1–P5 and G-I6-1…G-I6-8 all PASS.** **G-I6-6** proved the production
policy does not propose the anchor (55 kept + 11 proposed = 65 evaluated against 66 in the
repository); **G-I6-8 PASS on the first unattended cycle, 2026-08-20 03:00** (`629f3e84`) with
**all fourteen predictions observed** — count 66 → 67, groups unchanged at 43, the 16-path
group grown 2 → 3 with byte-identical path sets, `--dry-run` intact, **zero deletions**, the
would-remove set **unchanged at the same 11**, the anchor's record **byte-identical** to its
pre-cycle state, and the whole-repository delta exactly one new snapshot. **A hypothesis was
refuted, not smoothed over:** the dangling `parent: 4f4177e8…` is **not** the signature of a
prior retag — zero snapshots carried an `original` field before I-6 — so a snapshot really was
deleted by an unidentified mechanism, **after 2026-06-17 16:19**. **I-6 makes no claim against
that mechanism**; it protects the anchor from the normal nightly retention path.
Closeout: `09_logs/2026-08-20_I6_closeout.md`; dated pre-gate record
`09_logs/2026-08-20_I6_provisional_evidence.md`. Prior:
**I-5 — extend backup coverage (H-2) — COMPLETE 2026-08-19.** Three static paths appended to
the `PATHS` array of `/usr/local/bin/homelab-backup.sh` — `portainer_data/_data` (the only
copies of the stack definitions, H-3 / H-4), `/etc/cron.d/aurora-signals` and the openedai
`voice_to_speaker.yaml` — taking the recorded set from **13 to 16 paths** (installed script
sha256 `330df064…5895a554`). **G-I5-1…8, G-I5-10 and G-I5-11 closed at the attended run on
2026-08-18** (snapshot `afd3b4b4`), and **G-I5-9 closed on the first unattended cycle,
2026-08-19 03:00** (snapshot `ae45cd50`): 65 → **66 snapshots**, group count **unchanged at
43**, the 16-path group grown **1 → 2**, the new snapshot naming `afd3b4b4` as its parent,
**byte-identical 16-element path sets**, `--dry-run` intact and **zero real deletions**. The
last open item — no repository lock — was **closed by observation on 2026-08-19**, an
operator-run `restic list locks` returning no lock IDs, so **all eleven G-I5-9 predictions are
observed**. **G-I5-1…11 all PASS**, and exactly one carries an evidence qualification,
recorded rather than smoothed over: **G-I5-3** (the attended run's shell exit code was not
preserved, and no backup was re-run to manufacture one). **The secret half of H-2 remains
open as M-D.**
Closeout: `09_logs/2026-08-19_I5_closeout.md`; the dated pre-gate record is
`09_logs/2026-08-18_I5_provisional_evidence.md`. Prior:
...
repository locks**. Prerequisite for all of Program E. Closeout:
...
Last updated: 2026-08-19 (**I-5 — extend backup coverage (H-2) — COMPLETE.** The last open
gate, **G-I5-9**, closed on the first unattended cycle: 2026-08-19 03:00 produced snapshot
`ae45cd50`, which joined the new 16-path group naming `afd3b4b4` as its parent, with a
**byte-identical 16-element path set** — 65 → **66 snapshots**, group count **unchanged at
43**, `--dry-run` intact, **zero real deletions**, anchor `63c072f4` untouched, awareness
chain healthy. **The host never rebooted** (up since 2026-08-12 09:28), so the cron cycle ran
under exactly the preregistered conditions and no evidence was lost to the interrupted
session. The last prediction to close was the repository-lock check (needs
root) — **closed by observation the same day**, an operator-run `restic list locks` returning
no lock IDs. **G-I5-1…11 all PASS**; the single remaining evidence qualification in I-5 is
**G-I5-3** (attended-run exit code not preserved). **The non-secret half of H-2 is closed;
the secret half remains M-D, open.**
One earlier prediction is **corrected**: a `PATHS` change does **not** restart
the S-10 would-remove trail — the set was byte-identical across the boundary, and those 11
snapshots now sit in a group **frozen at 22 that no future snapshot can join**.
**Program E then advanced to I-6** (**COMPLETE 2026-08-20** — see the current entry above).
Documentation only; the
production change was made 2026-08-18. Closeout `09_logs/2026-08-19_I5_closeout.md`;
dated pre-gate record `09_logs/2026-08-18_I5_provisional_evidence.md`. Prior —
2026-08-17 (**Operational reconciliation after seventeen unattended days.**
...
was still required then; it CLOSED 2026-08-20 and the anchor is now `42506e44`**);
**(5)** the platform had read `degraded` since **2026-08-01** for a second,
...
**Nothing was fixed by that record** — S-9, M-1/M-A, S-10 and I-6 were all left Open, and
**I-5 was then the next milestone** (completed 2026-08-19; see the current entry above).
Record: `09_logs/2026-08-17_operational_reconciliation.md`.
...
Status: **Backup PASS — grouping defect FIXED at I-4 (2026-07-31); coverage EXTENDED at I-5
(2026-08-19); retention live but `--dry-run`, so no snapshot can be deleted.** The nightly
`restic backup` step has succeeded every night and recoverability is proven (restore drill
PASS E5-b 2026-06-27; empirical restic restore-drill G-F4-08 2026-07-27 on snapshot
`7715bf6a`).
**Re-verified 2026-08-19:** the I-4 fix has now held across **twenty further unattended
nights** — unbroken nightly snapshots from 2026-07-31 (`34def61f`) through 2026-08-19
(`ae45cd50`), parent detection working throughout, and neither the 2026-08-12 reboot nor the
I-5 path change disturbed the schedule. **G-I4-1…12 are not reopened** — this is continuing
evidence, not a re-gate.

- **Coverage was extended at I-5 (H-2 non-secret half — CLOSED 2026-08-19).** The recorded
  path set went from **13 to 16 static paths**; installed script sha256 is now
  **`330df064…5895a554`** (the I-4 value `90e8eb91…a907a45f` is retained only as the rollback
  reference at `/root/i5-evidence/homelab-backup.sh.pre-I5`). The three additions are
  `/var/lib/docker/volumes/portainer_data/_data`, `/etc/cron.d/aurora-signals` and
  `/srv/homelab/data/openedai-speech/voice_to_speaker.yaml`. All sixteen elements are static
  literals — **no date, timestamp or variable component**, which is the I-4 defect class and
  is checked by G-I5-2 and G-I5-9. Coverage was proven on **content**, not on the fact that a
  path was passed to restic: four stack definitions (`compose/{1,2,3,4}/docker-compose.yml`),
  the signal schedule and the voice map all appear in the snapshot's own file listing
  (**G-I5-6**).
- **I-5 started a new group, and that group is behaving.** Snapshot `afd3b4b4` (attended,
  2026-08-18) opened it; `ae45cd50` (unattended, 2026-08-19) joined it naming `afd3b4b4` as
  parent, with a byte-identical 16-element path set. **43 groups, 66 snapshots.**
- **Application-internal state inside a backed-up volume MAY be captured; independent secret
  stores remain excluded.** `portainer_data` is backed up whole — database, certificates and
  internal keys — because a selective exclusion would risk a partial Portainer restore
  (operator decision, 2026-08-18). `ai-stack/.env`, `/home/diego/.secrets/` and
  `/etc/restic/passwd-homelab` stay **out** and are **M-D**, open. Without the last of those
  the repository cannot be decrypted at all.
- **I-5 closeout (dated, authoritative for the change and its eleven gates):**
  [`../09_logs/2026-08-19_I5_closeout.md`](../09_logs/2026-08-19_I5_closeout.md).
...
- **Retention runs as `--dry-run`. No snapshot can be deleted by the nightly job.** This is
  ...
  decided (**S-10**). The anchor's half of that concern is now closed: **it has real
  protection as of I-6, 2026-08-20.** Re-enabling deletion is
...
- **The would-remove report arrived on 2026-08-05 — the prediction held.** I-4 expected it
  ...
  → 6 (08-12) → 9 (08-15) → 10 (08-17) → 11 (08-18)**, and **held at 11 on 2026-08-19**.
  **15 reports across 15 nights, and nothing was ever removed** — every one is phrased `Would have removed the following snapshots:`, restic's
  ...
  `09_logs/2026-08-17_operational_reconciliation.md` §5. **Updated 2026-08-19:** the trail
  reached **11** on 2026-08-18 and held at **11** across the I-5 boundary.
- **The I-5 path change did NOT reset the would-remove report — the earlier prediction was
  wrong, and is corrected here.** `restic forget` evaluates each `host,paths` group
  independently, so the *new group's* trail starts at zero while the report as a whole is
  untouched: the set was byte-identical before I-5 (2026-08-18 03:00), at the attended run
  (12:40) and after the first unattended 16-path cycle (2026-08-19 03:00), and the new group
  contributed **zero**. **Consequence for S-10:** all 11 proposed snapshots sit in a group
  **frozen at 22 members that no future snapshot can ever join** — a closed set, not a
  growing one. This strengthens the S-10 input rather than weakening it. Evidence:
...
- **The D-1.5 anchor is PROTECTED as of I-6 (2026-08-20) and is now `42506e44`.** It carries
  `["anchor","d15-rollback"]` and is **outside `--tag nightly` scope**, so the nightly policy
  does not evaluate it at all — proven at G-I6-6 and again at G-I6-8, where the policy
  evaluated **66** of the repository's **67** snapshots and the output contained **zero**
  occurrences of the anchor. **Until 2026-08-20 it survived by group shape alone** — appearing
  in none of the 15 would-remove reports because it sat in a legacy dated **2-member** group
  (with `5a5eadf2`) that no future snapshot can join. That was exactly the risk I-4 named
  (*never rely on group shape for protection*), and I-6 closed it.
  **`63c072f4` no longer names any snapshot**; the retag replaced it, and the old id is
  preserved in the new snapshot's `original` field, where it stays permanently resolvable.
  **Never run `restic tag` against `42506e44`** — that would change the id again.
- **The nightly retention policy has never removed a snapshot** since repository creation on
  ...
  removals.** **Open
  observation, raised 2026-07-31, sharpened at I-6 (2026-08-20):** the anchor records
  `parent: 4f4177e8…`, and no snapshot with that id exists in the repository. **A retag is
  NOT the explanation** — a full pre-I-6 metadata read showed **zero** snapshots carrying an
  `original` field, so nothing in this repository had ever been retagged. `4f4177e8` must have
  existed at **2026-06-17 16:19**, since restic selected it as the anchor's parent and parent
  selection requires a matching host and path set, making it a **third member of that dated
  group**. So *something* removed a snapshot — **after 2026-06-17 16:19**, not "on or before",
  as this document previously stated. Not the nightly policy, which was structurally inert
  throughout. **Still unexplained**, and exactly **one** dangling parent exists
  repository-wide, now inherited verbatim by `42506e44`. No I-4 or I-6 gate depends on it, and
  **I-6 makes no claim against that mechanism.**
- **Tag-based protection now exists for exactly one snapshot** — the D-1.5 anchor, since I-6
  (2026-08-20). No other snapshot has any protective mechanism (§5 of the incident record).
...
- **Backup coverage — H-2 is half closed.** The **non-secret half is CLOSED at I-5
  (2026-08-19)**: the Portainer volume, `/etc/cron.d/aurora-signals` and the openedai voice
  map are now inside the path set and verified present in a real snapshot. **The secret half
  remains open as M-D** — `ai-stack/.env`, `/home/diego/.secrets/` and
  `/etc/restic/passwd-homelab` are still outside it, so a root-disk loss is **not yet** fully
  recoverable to a running state from restic alone.
- **The backup mechanism is version-controlled as of I-8 (2026-08-20)** —
  `/usr/local/bin/homelab-backup.sh` **and** `/etc/cron.d/homelab-backup` are captured
  byte-identically under `07_operations/backups/`, which is the authority for install targets
  and drift checks. **This is Git durability, not restic coverage:** `07_operations/` is not
  in the path set, so both files remain **outside the backup**, and **H-2 is unchanged**. What
  Git adds that restic cannot is an **off-host** copy — the restic repository sits on the same
  physical machine. **Standing obligation:** whenever either live file changes, the tracked
  copy must be updated in the same change. The `diego` crontab remains uncovered and carries
  no part of the backup mechanism; no remediation identifier is assigned.
...
- Snapshot validated
- D-1.5 anchor snapshot retained as the
  pre-voice-pipeline rollback point (still in the
  repository, unchanged through D-1.6 / D-1.7 / D-1.8).
  **It is PROTECTED as of I-6 (2026-08-20) and its id is now `42506e44`** — tagged
  `["anchor","d15-rollback"]` and outside `--tag nightly` scope, so `forget --tag nightly`
  cannot reach it. Its data is untouched: same `time`, `tree`, `paths`, `hostname`.
  **Its historical id `63c072f4` no longer names a snapshot** and lives on in the
  `original` field. Before I-6 it was tagged `nightly` like every other snapshot, fully in
  scope for `forget --tag nightly`, and survived only because the policy deletes nothing
  (§5 of the incident record). Closeout: `09_logs/2026-08-20_I6_closeout.md`.
...
| **E** | Backup lifecycle | **I-4 COMPLETE 2026-07-31** (grouping fixed, Gate 8 closed on real evidence; retention held at `--dry-run`). **I-5 COMPLETE 2026-08-19** (13 → 16 paths; H-2 non-secret half closed; G-I5-9 PASS on the first unattended cycle). **I-6 COMPLETE 2026-08-20** (anchor protected at snapshot level; now `42506e44`, outside `--tag nightly` scope; G-I6-8 PASS on the first unattended cycle). **I-8 COMPLETE 2026-08-20** (script + cron captured byte-identically under `07_operations/backups/`; Recovery Artifacts; Git durability, **not** restic coverage — H-2 unchanged). Remaining: S-8 → S-10 |
...
- **I-4 — restic grouping defect FIXED 2026-07-31** (`3a47b21c`). Gate 8 closed on real
  evidence; retention deliberately held at `--dry-run`. Closeout:
  [`../09_logs/2026-07-31_I4_gate8_closeout.md`](../09_logs/2026-07-31_I4_gate8_closeout.md).
- **I-5 — backup coverage EXTENDED 2026-08-19.** 13 → **16 static paths**; the non-secret
  half of **H-2** is closed and verified on snapshot content, not on intent. **G-I5-9 PASS**
  on the first unattended cycle (`ae45cd50`, 2026-08-19 03:00): 66 snapshots, 43 groups
  unchanged, byte-identical path sets, parent detection intact, zero deletions; the
  repository-lock prediction closed **by observation** on 2026-08-19. **G-I5-1…11 all PASS**,
  with one stated evidence qualification (**G-I5-3**). **The secret half stays
  open as M-D.** Closeout:
  [`../09_logs/2026-08-19_I5_closeout.md`](../09_logs/2026-08-19_I5_closeout.md); dated
  pre-gate record:
  [`../09_logs/2026-08-18_I5_provisional_evidence.md`](../09_logs/2026-08-18_I5_provisional_evidence.md).
