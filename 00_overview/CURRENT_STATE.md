...
8. **ER-1 — Deterministic Entity Resolution** — **design FROZEN 2026-07-16 (now Revision 2);
   implementation in progress. ER-1.0 (`c147e632` → `38eb8262`), the Revision 2 amendment
   (`3ebf59d1`) and ER-1.1 (`f983a04f`) are committed + pushed; ER-1.2 (loader) is
   implemented + validated, at the git gate.** Natural-language requests do not resolve to real
   entity ids, and writes to non-existent entities in a live HA domain are reported as
   successful without verification (13 such calls in the real audit evidence). Spec:
