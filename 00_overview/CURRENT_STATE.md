...
8. **ER-1 — Deterministic Entity Resolution** — **design FROZEN 2026-07-16 (now Revision 3);
   ...
   (`3ebf59d1`), ER-1.1 (`f983a04f`) and ER-1.2 (`b0fded73`) are committed + pushed; G-ER-5
   CLOSED 2026-07-17 on real unattended evidence. ER-1.3 (projection emitter + D-ER-13 /
   Revision 3) implemented + validated 2026-07-17 — G-ER-6 producer half CLOSED. Next:
   ER-1.4a.** Natural-language requests do not resolve to real
   entity ids, and writes to non-existent entities in a live HA domain are reported as
   successful without verification (13 such calls in the real audit evidence). **Aurora's
   behaviour is unchanged so far — ER-1 changes reality at ER-1.4b, when ER-1-C1 lands.** Spec:
