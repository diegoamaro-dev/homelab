...
- rag_search
- audit_search
- `ha_get_state` (**v0.2.0** — ER-1.4a, 2026-07-17): resolves natural
  language to real entity ids via the ER-1.3 projection
  (`/opt/aurora/aurora-entities.json`). A canonical `entity_id` passes
  through to HA exactly as at v0.1.0 (D-ER-9); a non-id-shaped miss
  returns `unknown_entity` + candidates with no HA call. **Naming only
  — never authorization: D-12 remains the sole authority (INV-17).**
- `ha_call_service` (**v0.1.0** — unchanged; resolution + ER-1-C1 land
  at ER-1.4b)
...
8. **ER-1 — Deterministic Entity Resolution** — **design FROZEN 2026-07-16 (now Revision 3);
   implementation in progress. ER-1.0 (`c147e632` → `38eb8262`), the Revision 2 amendment
   ...
   validated 2026-07-17 — G-ER-7 read half + G-ER-6 consumer half (read side) CLOSED. Next:
   ER-1.4b.** **The read path now resolves natural language** (ER-1.4a); **the write path does
   not: `ha_call_service` is still v0.1.0, so writes to non-existent entities in a live HA
   domain are still reported as successful without verification** (13 such calls in the real
   audit evidence). **ER-1 changes that at ER-1.4b, when ER-1-C1 lands.** Spec:
...
9. **F-ER13-1 — `bin/aurora-context` can publish one file's content under another file's
   name.** Its `write_atomic` derives the temp path via `path.with_suffix(".tmp")`, so
...
10. **F-ER14-1 — the audit field `modelled` asserts more than it verifies. Operator decision
    wanted before ER-1.4b.** Spec §4 step 4 names the field and its own sentence scopes it to
    ...
    `modelled` and define it precisely in §4. **Wanted before ER-1.4b**, when the field starts
    describing actuation rather than reads. Record:
    `09_logs/2026-07-17_ER1_4a_ha_get_state_applied.md` §4.
